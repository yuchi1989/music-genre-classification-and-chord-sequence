...
def get_features5(file):
    ...
    a1 = calc_statistical_features(feats['scfp'].transpose())   #19*7 
    a1 = a1.reshape(a1.shape[0]*a1.shape[1])
    print a1.shape
    a2 = calc_statistical_features(feats['sfp'].transpose())    #19*7
    a2 = a2.reshape(a2.shape[0]*a2.shape[1])
    print a2.shape
    a3 = calc_statistical_features(feats['loudness'].transpose())  #24*7
    a3 = a3.reshape(a3.shape[0]*a3.shape[1])
    print a3.shape
    a4 = calc_statistical_features(tempogram)
    a4 = a4.reshape(a4.shape[0]*a4.shape[1])
    print a4.shape
    a5 = calc_statistical_features(chromagram)   #12*7
    a5 = a5.reshape(a5.shape[0]*a5.shape[1])
    print a5.shape
    feature5_set = np.hstack((a1,a2,a3,a4,a5))   #384*7
...
def testsvm():
    ...
    clf = SVC(C = 8, gamma = 1, kernel = 'poly', degree = 2)
    clf.fit(traindata, trainlabel)
    pred = clf.predict(testdata)
    print "svm classification accuracy: ", accuracy_score(testlabel,pred)
    cm = confusion_matrix(testlabel, pred)
