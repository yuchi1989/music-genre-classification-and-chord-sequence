...
def testsvm():
    ...
    Xtrain1,ytrain1,Xtest1,ytest1 = getSplitData1(p = 275) #175 0.68
    print "abc"
    ...
    for i in range(0,ntest):
        ...
        for j in range(0,ntrain):
            ...
            if(trainlabel[j] == 0):
                continue
            d = lcs(Xtest1[i],Xtrain1[j])
            if(d>dtemp2 and d<dtemp1):
                dtemp2 = d
                ptemp2 = ytrain1[j]
        #print Xtest[i]
        #print dtemp
        #print ptemp
        if ((ptemp*1+ptemp1*1+ptemp2*1)/3.0>1.5):
            ptemp = 2
        else:
            ptemp = 1
