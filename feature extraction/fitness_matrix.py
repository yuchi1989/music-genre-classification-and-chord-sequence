...
def read_audio(filename):
	...
	#print "soundinfo", len(sound_info)
...
def generate_filterbank(NFFT,fs,b,z):
	...
	for j in xrange(0,len(octpts)-2):
		#if(ctrrep[j+2] > bank.shape[1]):
			#ctrrep[j+2] = bank.shape[1]
		...
		y2 = (y/area)
		#print "y2", y2.shape
		print ctrrep[j],ctrrep[j+2]
		print "bank[]", bank[j,ctrrep[j]:ctrrep[j+2]].shape
...
file1 = "chadishijie.wav"
p, f, sound_info_old = read_audio(file1)
print "frequency is",f
print "channels are",p
print len(sound_info_old)
print len(sound_info_old)/f
...
Pxx, freqs, bins, im = spectrogram(sound_info, f, nfft1, 128)
...
bank = generate_filterbank(NFFT=nfft1,fs=f,b=24,z=6)
